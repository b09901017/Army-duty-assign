# 261 排勤務板 — 交接說明 v8（對應 index.html ≈ v42 · Apps Script v3 未動）
...
- **⚠️ 這節做過兩版，以「最終版」為準（v42）**：
  - **第一版（v41，已被取代）**：兩色用古銅/靛藍、標籤做成白字色籤浮在勤務方塊之上（z5）。使用者回饋「古銅跟打飯/分菜太像、靛藍跟公差太像會混淆」，且「色籤蓋到各人格子的字」。
  - **最終版（v42）**：
    1. **改中性色、明顯不撞任何勤務色**：`DAILY_RTN="#68727E"`（板岩灰＝準則日常）、`DAILY_ADH="#8C7B6A"`（暖褐＝臨時不計次）。勤務 8 色都是飽和色，這兩個中性色一眼區分。
    2. **名稱移到「左邊時間欄」**（不再浮在格子上）＝**不會遮到各人格子的字**。做法：`GUT` 由 34 加寬到 **58**，日常名稱右對齊畫在 `left:1,width:GUT-5`、`top+2`（格線下方）；時間軸的時間標在格線上方（`yy-6`）→ 時間在線上、名稱在線下，不互撞。色帶／虛線仍畫背景（z1，勤務蓋在上面）。
    3. **各人格子文字置中**：格子外層加 `display:flex;align-items:center;justify-content:center`、內層 `text-align:center`（line 1513 那塊）。
- 底部圖例改寫：灰帶=日常、暖褐=臨時提醒、紅線=現在、彩色方塊=各自勤務（名稱置中）。
...
- **目前 index.html ≈ v42、Apps Script v3（未動）。**
