...
`/tmp/tl_test.js`（45 項）＋ `/tmp/smoke.js`（各頁 render 不炸）為這次寫的。要重跑：把 harness 照 v4 第九節方式 stub（`localStorage/document(FakeEl)/window/navigator/location/setInterval…`、拿掉 `"use strict"`、`new Function(src+";return {...}")` 取內部函式）。**唯讀測記得 `store["duty-edit-ok"]="1"` 或直接 `state.readOnly=false`**，否則 `tlBlockInner` 不會給編輯控制項。
...
7. 補休/不在只在時間軸上方摘要，沒畫成方塊（避免 8 人的補休洗版）；`modeC` 才會把補休畫進各人欄。
