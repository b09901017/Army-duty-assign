...
## 六之四、v5 四次優化（排版與補休編輯的三點小修）

1. **沒重疊的方塊確實靠左**：`tlLanes` 舊版只比對每欄「最後一段的結束」，害不重疊的方塊被推到右欄。改成**比對整欄所有已放區間**是否重疊，沒重疊就進最左的空欄；重疊時仍照 `tlPri` 排左右。
2. **所有補休都能在時間軸點擊調時間**（不只自動補休）：手動排休（`absence`）的 block 帶 `src:{t:"absence",pid,range}`，點→ `tlrest` sheet 調時段，存檔用 `setAbsRange()` **回寫 `state.absence`** → 今日排休（`availCard`）跟著更新；「刪掉這筆」用 `delAbsRange()`。自動補休（duty/guard）維持存 `restRange`。
3. **自動補休也顯示在今日排休**：`availCard` 併入 `impliedRestsExpanded(state.duties,md)`，每人 chip 會標「補休HH:MM-HH:MM自」（帶「自」＝站哨/午打自動帶出、非手動排的）。
4. **方塊太窄看不到字→拉高最小軸距＋兩行不截斷**：`tlAxis` 的 `MIN` 30→46；方塊內容統一成**兩行**：上行標題（勤務名／「補休（午打/夜哨）」／「站哨」）、下行是誰，`word-break:break-all` 可換行不再 `ellipsis` 截字；右上角小標（×N／哨／臨／`自動✎`／人數）。
...
8. **時間軸重疊超過 3 欄會橫向捲動**（`wide=lc>3`，96px/欄）。打掃已收合（六之二第 6 點），但如果同時段還有很多「非打掃」的勤務重疊，還是會分欄橫捲。這是刻意（重疊本來就該並排）。若嫌太寬可把 `wide` 門檻調高改用百分比欄。
