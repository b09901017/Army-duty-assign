...
function guardAuto(){
  ...
  state.guard.days.forEach(function(day){
    ...
    day.shifts.forEach(function(sh){
      ...
      var sdate=sh.date||day.date,gsp=spanMin(guardRange(sh.code));   // 這班的日期＋時段（排除當天不在的人用）
      function avail(p){   // 排除當天整天不在／排休時段與這班重疊的人（站哨不再排給那天不在的人）
        if(fullDayOut(p.id,sdate))return false;
        if(gsp){var bad=false;absenceRecs(p.id,sdate).forEach(function(r){if(bad||!r.range)return;var rr=spanMin(r.range);if(rr&&gsp[0]<rr[1]&&gsp[1]>rr[0])bad=true;});if(bad)return false;}
        return true;
      }
      for(var c=0;c<need;c++){
        var cand=PEOPLE.filter(function(p){return !used[p.id]&&sh.assigned.indexOf(p.id)<0&&avail(p);});
        if(!cand.length)cand=PEOPLE.filter(function(p){return !used[p.id]&&sh.assigned.indexOf(p.id)<0;});   // 全被排除→退回不看當天狀況，至少排得出來
        if(!cand.length)cand=PEOPLE.filter(function(p){return sh.assigned.indexOf(p.id)<0;});
...
function autoAssign(){
  ...
  // 「誰有空」一律走 tlFreeAt（真實分鐘重疊；含整天不在/排休/自動補休(午打·夜哨·午哨)/站哨當班/其他已排勤務/全班261佔全員）
  // → 跟時間軸同一套真相；讀 live state.duties，邊排邊算、後面的勤務看得到前面剛排的人。
  function freeSet(span,exclId){var m={};tlFreeAt(span?span[0]:null,span?span[1]:null,md,exclId).forEach(function(x){m[x.pid]=x.free;});return m;}

  // 1) 打飯：輪流隊列；保留已排（手動/公版已填），只補空缺；挑「這餐時段有空」的人（會擋夜哨補休/站哨/排休）
  var q=state.mealQueue.slice(),mealShort=false;
  ...
  meals.forEach(function(m){
    var need=(m.count||1)-m.assigned.length;
    var free=freeSet(dutyEffSpan(m),m.id);
    for(var c=0;c<need;c++){
      var idx=-1;
      for(var i=0;i<q.length;i++){var pid=q[i];if(free[pid]&&m.assigned.indexOf(pid)<0){idx=i;break;}}
      if(idx<0){mealShort=true;break;}
      var pk=q[idx];m.assigned.push(pk);q.splice(idx,1);q.push(pk);
    }
  });

  // 2) 其他勤務：以「群組次數」為主平衡（大/小公差各自算）、其次同項、再其次整體非打飯總數
  function grp(d){return dutyGroup(d);}
  var grpWork={},catWork={},work={};
  PEOPLE.forEach(function(p){work[p.id]=nonMealTotal(p.id);});
  function gw(g,pid){if(!grpWork[g])grpWork[g]={};if(grpWork[g][pid]===undefined)grpWork[g][pid]=groupTotal(pid,g);return grpWork[g][pid];}
  ...
  // 預載本次已排（手動/公版已填）進公平度（跳過 keepAll、打飯、分菜標題）
  state.duties.forEach(function(d){if(d.removed||d.keepAll||d.kind==="meal"||d.fenca==="head")return;var g=grp(d);d.assigned.forEach(function(pid){work[pid]=(work[pid]||0)+1;grpWork[g]=grpWork[g]||{};grpWork[g][pid]=gw(g,pid)+1;catWork[d.label]=catWork[d.label]||{};catWork[d.label][pid]=cw(d.label,pid)+1;});});

  // 待排：可排的非打飯勤務（跳過 keepAll/prefilled/分菜標題/removed）
  var todo=state.duties.filter(function(d){return d.kind!=="meal"&&!d.removed&&!d.keepAll&&!d.prefilled&&d.fenca!=="head";});
  // issue 4：最受限先排（slack＝初始有空人數−還要排的人數；slack 小的先、還缺人多的先）→ 少誤報「人手不足」
  todo.forEach(function(d){var sp=dutyEffSpan(d),f=tlFreeAt(sp?sp[0]:null,sp?sp[1]:null,md,d.id),n=0;f.forEach(function(x){if(x.free)n++;});d._slack=n-(d.count-d.assigned.length);});
  todo.sort(function(a,b){return a._slack-b._slack||(b.count-b.assigned.length)-(a.count-a.assigned.length);});

  var short=false;
  todo.forEach(function(d){
    var g=grp(d),cat=d.label,need=d.count-d.assigned.length,free=freeSet(dutyEffSpan(d),d.id);   // 同一項的空位共用一次 freeSet（非打飯勤務不會改到同時段別人的空閒）
    for(var i=0;i<need;i++){
      var cand=PEOPLE.filter(function(p){return free[p.id]&&d.assigned.indexOf(p.id)<0;});
      if(!cand.length){short=true;break;}
      cand.sort(function(a,c){return gw(g,a.id)-gw(g,c.id)||cw(cat,a.id)-cw(cat,c.id)||work[a.id]-work[c.id]||a.id-c.id;});
      var pk=cand[0].id;d.assigned.push(pk);work[pk]=(work[pk]||0)+1;
      grpWork[g]=grpWork[g]||{};grpWork[g][pk]=gw(g,pk)+1;
      catWork[cat]=catWork[cat]||{};catWork[cat][pk]=cw(cat,pk)+1;
    }
    delete d._slack;
  });
  flash((mealShort||short)?"已分配，但有時段人手不足":"已分配（打飯輪流·勤務均分）");render();
}
function commit(){
  var entries=[];
  state.duties.forEach(function(d){
    if(d.removed||d.fenca==="head")return;   // 分菜「標題行」只是區塊標記，不排人、不計入
